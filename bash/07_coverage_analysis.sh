...
do
  ...
  do
   ...
   '| sbatch -J $n.$seq --ntasks=1 --cpus-per-task=1 --time=5:00:00 --mem-per-cpu=6G --account=kdm16_sc_default --partition=sla-prio --out slurm/density.$n.$seq.%j.out
  done
done
...
do
  ...
  do
    intersectBed -a ref/v0.4_dip_$seq.pri.cov.bed -b coverage/$prefix.$n.$seq.bed -wo -namecheck|\
