...
DATA3 <- sumtib %>% filter(Class!="lncrna") %>%
  mutate(PrintName=case_when(#Class=="lncrna" ~ "lncRNA",  
                            Class=="promoter" ~ "1 kb upstream\nfrom TSS",
                            Class=="all_repeats" ~ "Repeats",
                            Class=="introns" ~ "Intronic",
...
p3<-make_densityplot(SUB%>%filter(Class=="CDS"), TRUE, TRUE, FALSE)
p4<-make_densityplot(SUB%>%filter(Class=="UTR3"), FALSE, FALSE, FALSE)
...
pcomb2<-p1+p2+p3+p4+p5 + plot_layout(widths=c(1,1,1,1,1))+plot_annotation(title = "")
pcomb2
outfile=paste(plotdir,"FigS12_Methylation_density_StrandSpec.png", sep="")
...
outfile=paste(plotdir,"FigS13_Piechart_Regions_with_or_without_CpG_Signif.png", sep="")
