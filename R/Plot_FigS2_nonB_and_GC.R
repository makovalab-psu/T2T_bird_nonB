...
# chicken
cdensfile="coverage/chicken.v23.per_chrom.tsv"
cgcfile="stats/chicken.v23.GC_per_chrom.txt"
cgroupfile="helpfiles/chicken.v23.groups.txt"

# Reading in the data 
dens_tib<-densfile %>% read.table(header=TRUE) %>% as_tibble() %>%
  mutate(NonB=if_else(NonB=="Any", "All", NonB))
...
cgrouptib <- cgroupfile %>% read.table(header=FALSE) %>% 
  as_tibble() %>% rename(Chr=V1, Length=V2, Group=V3) %>% 
  mutate(Group=case_when(Group=="macro" ~ "Macro",
                         Group=="micro" ~ "Micro",
                         Group=="dot" ~ "Dot",
...
CDATA <- cdens_tib %>% inner_join(cgc_tib) %>% inner_join(cgrouptib)
CDATA$NonB <- factor(CDATA$NonB, levels=c("All","APR", "DR", "STR", "IR", "TRI", "G4", "Z"))
CDATA$Group <- factor(CDATA$Group, levels=c("Macro", "Micro", "Dot"))
...
make_plot <- function(data, title=NULL) {
  ...
  p<-ggplot(data, aes(x=GC_cont*100, y=Coverage*100, fill=Group, color=Group))+
    ...
    labs(x="GC content (%)", y="Non-B motif content (%)", title=title, scale="free_y") +
    scale_fill_manual(values=type_col)+
    scale_color_manual(values=type_col)+
    scale_shape_manual(values=c(21,24,22))+
    facet_wrap(NonB~., ncol=2, scales = "free_y")+
    expand_limits(y = 0) +
...
outfile=paste(plotdir,"Figure_S2_NonB_GC_ZFandChicken.png", sep="")
ggsave(outfile,plot = pcomb,scale = 1,dpi = 300,limitsize = TRUE,width=8,height=10)
outfile=paste(plotdir,"Figure_S2_NonB_GC_ZFandChicken.svg", sep="")
ggsave(outfile,plot = pcomb,scale = 1,dpi = 600,limitsize = TRUE,width=8,height=10)
