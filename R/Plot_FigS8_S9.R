...
# Input files 
file="methylation/functional/Group.allCpG.merged.txt"
intergenfile="methylation/functional/Group.allCpG.intergenic.txt"
sumfile<-"methylation/functional/Group.summaryCpG.merged.txt"
...
vcolors=viridis(8)
cat_col=c("gray10",vcolors[6])
meth_colors=c("gray10",vcolors[7], vcolors[7])
################################################################################
# PLOT
...
p5<-make_densityplot(SUB%>%filter(Class=="intronic"), FALSE, FALSE, FALSE)
p6<-make_densityplot(SUB%>%filter(Class=="nonprotcoding"), FALSE, FALSE, FALSE, TRUE)
...
ggsave(outfile,plot = pcomb2,scale = 1,dpi = 600,limitsize = TRUE,width=10,height=6)
...
SIGN$MethClass <- factor(SIGN$MethClass, levels=c("No", "Macro","Micro","Dot"))
#DATA3$PrintName <- factor(DATA3$PrintName, levels=c("1 kb upstream\nfrom TSS","5'UTRs","CDS", "3'UTRs", "Intronic", "lncRNA", "Intergenic"))
SIGN$PrintType <- factor(SIGN$PrintType, levels=c("Background","Coding","Template"))
...
ggsave(outfile,plot = p3,scale = 1,dpi = 600,limitsize = TRUE,width=7,height=10)
