...
# SUPPLEMENTARY FIGURE 15 COMPARE Z-DNA ANNOTATION (NO BACKGROUND)
...
# SUPPLEMENTARY FIGURE 16 CHICKEN
...
make_vertical<-function(data) {
  ...
  # Combine 
  pcomb<-cowplot::plot_grid(p, leg, ncol=1,rel_heights=c(1, 0.15)) # changed this for chicken
...
# Zebra finch - genomewide base line
zfgw_sign<-calc_signif(tib%>%filter(Set=="GW"),bgtib)
DATA13<-make_table(tib%>%filter(Set=="GW"),zfgw_sign,chr_vect, nonBvect) %>% drop_na() 

# Zebra finch - chromosome baseline
zfchr_sign<-calc_signif(tib%>%filter(Set=="CHR"),bgtib)
DATAS14<-make_table(tib%>%filter(Set=="CHR"),zfchr_sign,chr_vect, nonBvect) %>% drop_na() 

# Zebra finch - genome wide base line with alt Z-DNA annotation
zdna_zfgw_sign<-calc_signif(ztib%>%filter(Set=="GW"),zbgtib)
DATAS15<-make_table(ztib%>%filter(Set=="GW"),zdna_zfgw_sign,chr_vect,Zvect) %>% drop_na() 

# Chicken - genomewide baseline
chgw_sign<-calc_signif(ctib%>%filter(Set=="GW"),cbgtib)
DATA16<-make_table(ctib%>%filter(Set=="GW"),chgw_sign,cchr_vect, nonBvect) %>% drop_na()
...
p13<-make_vertical(DATA13)
p13
outfile1=paste(plotdir,"FigS13_ZFcentromere.png", sep="")
ggsave(outfile1,plot = p13, bg = "white",scale = 1,dpi = 300,limitsize = TRUE,height=9,width=3.9)

# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# FIGURE S14 
p14<-make_vertical(DATAS14)
p14
outfile2=paste(plotdir,"FigS14_ZFcentromere_chrbaseline.png", sep="")
ggsave(outfile2,plot = p14, bg = "white",scale = 1,dpi = 300,limitsize = TRUE,width=3.9,height=9)

# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# FIGURE S15
p22<-make_vertical(DATAS15)
p22
outfile3=paste(plotdir,"FigS15_ZFcentromere.ZDNA.png", sep="")
ggsave(outfile3,plot = p22,bg = "white", scale = 1,dpi = 300,limitsize = TRUE,width=6,height=9)

# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# FIGURE S15
p16<-make_vertical(DATA16)
p16
outfile4=paste(plotdir,"FigS16_GGcentromere.png", sep="")
ggsave(outfile4,plot = p16, bg = "white",scale = 1,dpi = 300,limitsize = TRUE,width=3.9,height=5)
...
################################################################################
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# Make a function that plots the data horizontally - this was not used for the 
# final paper! 
