...
DATAH$NonB <- factor(DATAH$NonB, levels=c("All","APR", "DR", "STR", "IR", "TRI", "G4", "Z"))
DATAO$Group <- factor(DATAO$Group, levels=c("Macro","Micro","Dot"))
DATAO$NonB <- factor(DATAO$NonB, levels=c("All","APR", "DR", "STR", "IR", "TRI", "G4", "Z"))
...
p1
outfile=paste(plotdir,"FigS24_nonB_vs_coverage.hifi.macromicro.png", sep="")
ggsave(outfile,plot = p1,scale = 1,dpi = 300,limitsize = TRUE,width=10,height=6)


p2<-make_plot(DATAO, "ONT")
p2
outfile=paste(plotdir,"FigS24_nonB_vs_coverage.ONT.macromicro.png", sep="")
