...
# S20
zfgw_sign<-calc_signif(tib%>%filter(Set=="GW"),bgtib)
DATAS20<-make_table(tib%>%filter(Set=="GW"),zfgw_sign,chr_vect, nonBvect) %>% drop_na() 

# S21
zfchr_sign<-calc_signif(tib%>%filter(Set=="CHR"),bgtib)
DATAS21<-make_table(tib%>%filter(Set=="CHR"),zfchr_sign,chr_vect, nonBvect) %>% drop_na() 

# S22
zdna_zfgw_sign<-calc_signif(ztib%>%filter(Set=="GW"),zbgtib)
DATAS22<-make_table(ztib%>%filter(Set=="GW"),zdna_zfgw_sign,chr_vect,Zvect) %>% drop_na() 
...
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# FIGURE S20 
ps20<-make_vertical(DATAS20)
outfile2=paste(plotdir,"FigS20_centromere_bothHaplo.png", sep="")
ggsave(outfile2,plot = ps15, bg = "white",scale = 1,dpi = 300,limitsize = TRUE,width=3.9,height=9)


# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# FIGURE S21
ps21<-make_vertical(DATAS21)
outfile=paste(plotdir,"FigS21_centromere_vs_chrLevels.png", sep="")
ggsave(outfile,plot = ps16,bg = "white", scale = 1,dpi = 300,limitsize = TRUE,width=3.9,height=9)

  
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# FIGURE S22
ps22<-make_vertical(DATAS22)
ps22
outfile=paste(plotdir,"FigS22_centromere.ZDNA.png", sep="")
ggsave(outfile,plot = ps22,bg = "white", scale = 1,dpi = 300,limitsize = TRUE,width=6,height=9)
