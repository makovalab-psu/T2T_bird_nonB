...
genefile="coverage/bTaeGut7v0.4_MT_rDNA.coding.100kb.bed"
...
DATA1<-windfile %>% read.table(header=TRUE) %>% as_tibble() %>% 
  ...
  group_by(NonB) 
DATA1$NonB <- factor(DATA1$NonB, levels=c("APR", "DR", "STR", "IR", "TRI", "G4", "Z"))
ABC<- ABfile %>% read.table(header=FALSE) %>% as_tibble() %>% select(V1,V2,V3,V4) %>%
  rename(Chr=V1, Start=V2, Stop=V3,Compartment=V4) %>% bind_rows(centib) %>%
  mutate(PrintName=str_replace_all(Chr, "_mat", ""), 
         PrintName=str_replace_all(PrintName, "_pat", ""), 
         PrintName=str_replace_all(PrintName, "chr", "Chr")) 

# Other features (GC, repeats, genes)
...
DATA2<-genetib %>% bind_rows(gctib) %>% bind_rows(reptib) %>% 
  rename(Chr=V1, Start=V2, Stop=V3,Orig=V4) %>% mutate(midpoint=Start+(Stop-Start)/2) %>%
  mutate(PrintName=str_replace_all(Chr, "_mat", ""), 
         PrintName=str_replace_all(PrintName, "_pat", ""), 
         PrintName=str_replace_all(PrintName, "chr", "Chr"))
DATA2$Type <- factor(DATA2$Type, levels=c("GC", "Genes", "Repeats"))



################################################################################
make_landscape_plot<-function(data1, data2, data3, list) {
  ...
  # Other features 
  p2<-ggplot(data2, aes(x=midpoint/1000000, y=Cov)) +
    geom_line(show.legend = FALSE, color="grey") +
    geom_area(alpha = 0.7, show.legend = FALSE, fill="grey") +
    facet_grid(Type~PrintName, scales="free", space="free_x")+
    theme(panel.background = element_rect(fill = 'white', colour="black"),
          panel.grid = element_blank(),
          strip.background = element_blank(),
          plot.title = element_blank(),
          strip.text.y = element_text(size=8, hjust=0.5, angle=-30),
  ...
  p_dummy <- ggplot() +
    ...
    geom_rect(data=data3, aes(xmin=Start/1e6, xmax=Stop/1e6, ymin=0.2, ymax=1, fill=Compartment), color=NA, show.legend = TRUE) +
    facet_grid(.~PrintName, scales="free_x", space="free_x") +
    scale_fill_manual(values=c("#E38AAA7f","#6F9DD07f", "red"))+
    geom_point(
      data = data3%>%filter(Compartment=="CEN"),
  ...
  p<- p1/ p2/ plot_spacer() / p_dummy  + plot_layout(heights = c(5,2, -0.5, 0.2)) 
  return(p)
} 
...
for (i in 1:length(ALL)) {
  ...
  # Subsample  
  SUB1 <- DATA1 %>% filter(Chr %in% l) %>% 
    mutate(max=max(Dens)/1000)
  SUB1$PrintName<-factor(SUB1$PrintName, levels = namelist)
  SUB2 <- DATA2 %>% filter(Chr %in% l)
  SUB2$PrintName<-factor(SUB2$PrintName, levels = namelist)
  
  # Add AB + cen regions 
  ABSUB<- ABC %>% filter(Chr %in% l) %>% 
    select(Chr,Start,Stop,PrintName,Compartment)
  ABSUB$PrintName<-factor(ABSUB$PrintName, levels = namelist)
  
  plots[[i]] <- make_landscape_plot(SUB1, SUB2, ABSUB, namelist)
...
page1 <- (wrap_elements(plots[[1]]+theme(legend.position="none")) / wrap_elements(plots[[2]]+theme(legend.position="none")) / wrap_elements(plots[[3]])) + plot_layout(heights=c(1,1,1.05))
page2 <- (wrap_elements(plots[[4]]+theme(legend.position="none")) / wrap_elements(plots[[5]]+theme(legend.position="none")) / wrap_elements(plots[[6]])) + plot_layout(heights=c(1,1,1))
page3 <- (wrap_elements(plots[[7]]+theme(legend.position="none")) / wrap_elements(plots[[8]]+theme(legend.position="none")) / wrap_elements(plots[[9]])) + plot_layout(heights=c(1,1,1))
page4 <- wrap_elements(plots[[10]])


outfile=paste(plotdir,"FigS3_part1_nonBcontent.png", sep="")
ggsave(outfile,plot = page1,scale = 1,dpi = 300,limitsize = TRUE,width=10,height=13)
outfile=paste(plotdir,"FigS3_part2_nonBcontent.png", sep="")
ggsave(outfile,plot = page2,scale = 1,dpi = 300,limitsize = TRUE,width=10,height=13)
outfile=paste(plotdir,"FigS3_part3_nonBcontent.png", sep="")
ggsave(outfile,plot = page3,scale = 1,dpi = 300,limitsize = TRUE,width=10,height=13)
outfile=paste(plotdir,"FigS3_part4_nonBcontent.png", sep="")
ggsave(outfile,plot = page4,scale = 1,dpi = 300,limitsize = TRUE,width=10,height=5)
