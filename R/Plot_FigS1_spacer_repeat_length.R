...
make_plot <- function(data, header, c, verti=NULL) { 
  ...
  pcomb<-p_top +plot_spacer() + p_main + p_right +plot_layout(ncol=2, nrow=2, heights=c(1,3), widths=c(3,2))
  return(pcomb)
} 
...
outfile=paste(plotdir,"FigS24_Spacer_vs_repeat.png", sep="")
ggsave(outfile,plot = p123,scale = 1,dpi = 300,limitsize = TRUE,width=7,height=12)
