...
make_landscape_plot<-function(data1, data2, list) {
  ...
  p_dummy <- ggplot() +
    ...
    geom_rect(data=data2, aes(xmin=Start/1e6, xmax=Stop/1e6, ymin=0.2, ymax=1, fill=Compartment), color=NA, show.legend = TRUE) +
    facet_grid(.~PrintName, scales="free_x", space="free_x") +
    scale_fill_manual(values=c("#E38AAA7f","#6F9DD07f", "red"))+
    geom_point(
      data = data2%>%filter(Compartment=="CEN"),
  ...
  p<- p1/ plot_spacer() / p_dummy  + plot_layout(heights = c(5,-0.5, 0.2)) 
  return(p)
...
page1 <- (wrap_elements(plots[[1]]+theme(legend.position="none")) / wrap_elements(plots[[2]]+theme(legend.position="none")) / wrap_elements(plots[[3]])) + plot_layout(heights=c(1,1,1))
