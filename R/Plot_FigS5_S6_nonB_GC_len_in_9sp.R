################################################################################
### R code for plotting Non-B DNA motif content vs GC and length for 9 species
...
pgrid1<-(plots[[1]] + plots[[2]]+ plots[[5]]) / (plots[[6]] + plots[[3]]+ plots[[4]]) / (plots[[9]] + plots[[8]] + plots[[7]])
pgrid2<-(plots_len[[1]] + plots_len[[2]]+ plots_len[[5]]) / (plots_len[[6]] + plots_len[[3]]+ plots_len[[4]]) / (plots_len[[9]] + plots_len[[8]] + plots_len[[7]])
...
# Non B vs GC
p1_with_y <- (y_axis | pgrid1) +
  plot_layout(widths = c(0.01, 1))
pcomb1_base <- p1_with_y /
  x_axis1 +
  plot_layout(heights = c(1, 0.05))
# Now add legend on top
pcomb1 <- pcomb1_base +
  inset_element(legend_plot,
                left = 0.72, bottom = 0.0, right = 1.0, top = 0.08,
                align_to = "full")
pcomb1


# NonB vs Len
p2_with_y <- (y_axis | pgrid2) +
  plot_layout(widths = c(0.01, 1))
pcomb2_base <- p2_with_y /
  x_axis2 +
...
# Save plots
outfile=paste(plotdir,"FigS6_NonB_GC_all_species.png", sep="")
