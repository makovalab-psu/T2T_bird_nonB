################################################################################
### R code for plotting non-B motif overlaps in Zebra finch.
...
fileMulti="overlap/bTaeGut7v0.4_MT_rDNA.merged.7types.summary.txt"
filePair="overlap/bTaeGut7v0.4_MT_rDNA.merged.7types.pairwise.txt"
...
tibP<-filePair %>% read.table(header=TRUE) %>% as_tibble() %>% 
  ...
  mutate(PrintName=case_when(Region=="autosomes" ~ "Autosomes",
                       Region=="chrW_mat" ~ "Chromosome W",
                       Region=="chrZ_pat" ~ "Chromosome Z",
                       Region=="macro" ~ "Macro",
                       Region=="micro" ~ "Micro",
                       Region=="dot" ~ "Dot"))
tibP$NB1=factor(tibP$NB1, levels=c("APR", "DR", "STR", "IR", "TRI", "G4", "Z"))
tibP$NB2=factor(tibP$NB2, levels=c("APR", "DR", "STR", "IR", "TRI", "G4", "Z"))
...
SUBM <- tibM %>% filter(Bp>10000) %>% 
  filter(!str_detect(PrintName, "some"))%>% arrange(desc(Bp)) %>% 
 # filter(str_detect(PrintName, "some"))%>% arrange(desc(Bp)) %>% 
...
SUBP<-tibP %>% filter(!str_detect(PrintName, "some"))
#SUBP<-tibP %>% filter(str_detect(PrintName, "some"))
