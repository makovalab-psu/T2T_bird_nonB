...
# Input files 
hififile="densities/bTaeGut7v0.4_MT_rDNA.hifi.nonB_and_seqCov.bed"
ontfile="densities/bTaeGut7v0.4_MT_rDNA.ont.nonB_and_seqCov.bed"
...
DATAH$NonB <- factor(DATAH$NonB, levels=c("ALL","APR", "DR", "STR", "IR", "MR","TRI", "G4", "Z"))
DATAO$Group <- factor(DATAO$Group, levels=c("Macro","Micro","Dot"))
DATAO$NonB <- factor(DATAO$NonB, levels=c("ALL","APR", "DR", "STR", "IR", "MR","TRI", "G4", "Z"))
...
p1
outfile=paste(plotdir,"FigS17_nonB_vs_coverage.hifi.macromicro.png", sep="")
ggsave(outfile,plot = p1,scale = 1,dpi = 300,limitsize = TRUE,width=10,height=6)


p2<-make_plot(DATAO, "ONT")
p2
outfile=paste(plotdir,"FigS18_nonB_vs_coverage.ONT.macromicro.png", sep="")
